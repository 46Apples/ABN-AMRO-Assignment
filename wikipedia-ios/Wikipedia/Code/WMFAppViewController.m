...
- (BOOL)processUserActivity:(NSUserActivity *)activity animated:(BOOL)animated completion:(dispatch_block_t)done {
    ...
    switch (type) {
        ...
        case WMFUserActivityTypePlaces: {
            ...
            WMFMapRegionCoordinates *mapCoordinates = activity.wmf_mapCoordinates;
            
            if (mapCoordinates.isValid) {
                [[self placesViewController] updateViewModeToMap];
                [[self placesViewController] showLocationWithLatitude:[mapCoordinates.wmf_mapLattitude doubleValue] longitude:[mapCoordinates.wmf_mapLongitude doubleValue]];
            }
        } break;
        case WMFUserActivityTypeContent: {
            [self dismissPresentedViewControllers];
