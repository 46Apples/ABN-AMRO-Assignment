...
+ (instancetype)wmf_placesActivityWithURL:(NSURL *)activityURL {
    ...
    [tempUserInfo addEntriesFromDictionary:activity.userInfo];
    
    for (NSURLQueryItem *item in components.queryItems) {
        if ([item.name isEqualToString:@"WMFArticleURL"]) {
            NSString *articleURLString = item.value;
            articleURL = [NSURL URLWithString:articleURLString];
        } else if ([item.name isEqualToString:@"lattitude"]) {
            [tempUserInfo addEntriesFromDictionary:@{@"WMFMapLattitude": item.value}];
        } else if ([item.name isEqualToString:@"longitude"]) {
            [tempUserInfo addEntriesFromDictionary:@{@"WMFMapLongitude": item.value}];
        }
    }
    
    activity.webpageURL = articleURL;
    
    activity.userInfo = tempUserInfo;
...
- (WMFMapRegionCoordinates *)wmf_mapCoordinates {
    WMFMapRegionCoordinates *result = [WMFMapRegionCoordinates new];
    
    result.wmf_mapLattitude = self.userInfo[@"WMFMapLattitude"];
    result.wmf_mapLongitude = self.userInfo[@"WMFMapLongitude"];
